...
public enum Pitch {
    ...
    public String getABCValue(int key, int realKey) {
        if (key != realKey) {
            ...
            if (realKeyIndex < division1[pitchIndex] && keyIndex >= division1[pitchIndex])
                str = "_";
            else if (realKeyIndex > division2[pitchIndex] && keyIndex <= division2[pitchIndex])
                str = "^";
            else if ((realKeyIndex >= division1[pitchIndex] && realKeyIndex <= division2[pitchIndex]) &&
                    (keyIndex < division1[pitchIndex] || keyIndex > division2[pitchIndex]))
                str = "=";
        ...
        if (key == 0 || key == 2 || key == 4 || key == 7 || key == 9 || key == 11)
            return valuesSharp[pitch];
